...
class ListEmployeeActivityRequests extends ListRecords
{
    use HasAdministrationFormFeedback;

    protected static string $resource = EmployeeActivityRequestResource::class;

    protected static ?string $title = 'PERMOHONAN PENONAKTIFAN GURU DAN PEGAWAI';

    public function content(Schema $schema): Schema
    {
        return $schema
            ->components([
                View::make('filament.admin.resources.employee-activity-requests.requirements-panel')
                    ->viewData(fn (): array => [
                        'requirements' => ActivityRequirement::query()
    ...
    public function editRequirementsAction(): Action
    {
        return Action::make('editRequirements')
            ...
            ->color('warning')
            ->modalHeading('Edit Dokumen Pengajuan Penonaktifan')
            ->modalDescription('Tambah, hapus, urutkan, atau ubah keterangan dokumen.')
            ->modalSubmitActionLabel('Simpan Perubahan')
            ->modalWidth(Width::SevenExtraLarge)
            ->visible(fn (): bool => auth()->user()?->can('approval.update') ?? false)
            ->fillForm(fn (): array => [
                'items' => ActivityRequirement::query()
                    ->ordered()
                    ->get()
                    ->map(fn (ActivityRequirement $requirement): array => [
            ...
            ->schema([
                Repeater::make('items')
                    ...
                    ->columns(6)
                    ->defaultItems(0)
                    ->addActionLabel('Tambah Dokumen/Keterangan')
                    ->reorderable()
                    ->collapsible()
                    ->itemLabel(
                        fn (array $state): string => str($state['description'] ?? 'Keterangan baru')
                            ->limit(80)
                            ->toString(),
                    )
                    ->columnSpanFull(),
            ...
            ->action(function (array $data): void {
                ...
                Notification::make()
                    ->title('Dokumen permohonan berhasil diperbarui')
                    ->success()
                    ->send();
            });
    }
    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('create')
                ...
                ->url(EmployeeActivityRequestResource::getUrl('create', isAbsolute: false)),
        ];
    }
        ...
        DB::transaction(function () use ($items): void {
            ...
            foreach (array_values($items) as $index => $item) {
                $requirement = filled($item['id'] ?? null)
                    ? ActivityRequirement::query()->findOrFail($item['id'])
                    : new ActivityRequirement(['created_by' => auth()->id()]);
