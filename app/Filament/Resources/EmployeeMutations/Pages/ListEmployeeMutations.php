...
class ListEmployeeMutations extends ListRecords
{
    ...
    protected static ?string $title = 'MUTASI GURU & PEGAWAI';

    public function content(Schema $schema): Schema
    {
        return $schema
            ->components([
                View::make('filament.admin.resources.employee-mutations.requirements-panel')
                    ->viewData(fn (): array => [
                        'requirements' => MutationRequirement::query()
    ...
    public function editRequirementsAction(): Action
    {
        return Action::make('editRequirements')
            ...
            ->color('warning')
            ->modalHeading('Edit Kelengkapan Usulan Mutasi')
            ->modalDescription('Tambah, hapus, urutkan, atau ubah keterangan kelengkapan mutasi.')
            ->modalSubmitActionLabel('Simpan Perubahan')
            ->modalWidth(Width::SevenExtraLarge)
            ->visible(fn (): bool => auth()->user()?->can('approval.update') ?? false)
            ->fillForm(fn (): array => [
                'items' => MutationRequirement::query()
                    ->ordered()
                    ->get()
                    ->map(fn (MutationRequirement $requirement): array => [
            ...
            ->action(function (array $data): void {
                ...
                Notification::make()
                    ->title('Kelengkapan mutasi berhasil diperbarui')
                    ->success()
                    ->send();
            });
    }
    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('create')
                ...
                ->url(EmployeeMutationResource::getUrl('create', isAbsolute: false)),
        ];
    }
        ...
        DB::transaction(function () use ($items): void {
            ...
            foreach (array_values($items) as $index => $item) {
                $requirement = filled($item['id'] ?? null)
                    ? MutationRequirement::query()->findOrFail($item['id'])
                    : new MutationRequirement(['created_by' => auth()->id()]);
