...
class ActivityRequirement extends Model
{
    ...
    {
        static::updated(function (ActivityRequirement $requirement): void {
            ...
            if (filled($oldPath) && ! str_starts_with($oldPath, 'http') && $oldPath !== $requirement->template_path) {
                Storage::disk(EmployeeActivityRequest::DISK)->delete($oldPath);
            }
        });

        static::deleted(function (ActivityRequirement $requirement): void {
            if (filled($requirement->template_path) && ! str_starts_with($requirement->template_path, 'http')) {
                Storage::disk(EmployeeActivityRequest::DISK)->delete($requirement->template_path);
            }
        });
    }
